...
server <- function(input, output, session){
  ...
  #### making slider reactive to everytime the step size changes #######
  steps <- reactive({
    as.numeric(input$step_size)
  })
  
  #### making slider reactive to everytime the speed of the animation changes ######
  speedInput <- reactive({
    switch(input$speed,
           "Slow" = 3000,
           "Medium" = 1000,
  ...
  output$slider <- renderUI({
    sliderInput("timeRange", label = "Date/Time:", width = '600px',
                ...
                step = 60*steps(),
                timezone = "UTC",
                animate = animationOptions(interval = speedInput(),
  ...
  ## updates to the slider - so to retain the value of the #########
  ## slider if any of the controls are changed
